...
private:
  ...
  get_tiles_weights(const std::vector<Tile<T>> &tiles) {
    ...
    for (size_t i = 0; i < tiles.size(); ++i) {
      for (size_t j = 0; j < tiles[i].data.size(); ++j) {
        frequencies.push_back(tiles[i].weight / tiles[i].data.size());/*权重/总量 = 该tile的数量？*/
      }
    }
    return frequencies;
  }
        ...
        for (unsigned y = 0; y < size; y++) {
          for (unsigned x = 0; x < size; x++) {
            tiling.get(i * size + y, j * size + x) =
                tiles[oriented_tile.first].data[oriented_tile.second].get(y, x);/*一个个的tile拼接成一个完整的图？*/
          }
        }
      }
    }
  ...
  void set_tile(unsigned tile_id, unsigned i, unsigned j) noexcept {
    for (unsigned p = 0; p < id_to_oriented_tile.size(); p++) {
      if (tile_id != p) {
        wfc.remove_wave_pattern(i, j, p);/*把其他的都移除就相当于设为指定的？*/
      }
    }
  }
